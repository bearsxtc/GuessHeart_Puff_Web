 ...
 * @author Admin
 */
@WebServlet(name = "OrderPaidServlet", urlPatterns = {"/member/order_paid.do"})
public class OrderPaidServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(errors.isEmpty()){
            try{
                ...
                Order_Order order_order = service.selectOrder_OrderById(orderId);
                if(member.equals(order_order.getMember())){
                    service.updateOrder_OrderStatusToTransfered(member.getEmail(),orderId, bank, last5Code, amount, date, time);                    
                    //3.1 redirect到歷史訂單
                    response.sendRedirect("order_orders_history.jsp");
                    return;
                }else{
                    errors.add("預訂訂單編號不正確");
                }   
        ...
        request.getRequestDispatcher("order_paid.jsp").forward(request, response);
