    ...
    <body>
        ...
        <div id="article">     
            <div id="main">
                ...
                <hr id="hr">
                <%if (order != null) {%>
                <form action="paid.do" method="GET">
                    ${requestScope.errors}
                    <p>
                        <label>訂單編號: </label>
                        <input hidden name="orderId" value="<%= order.getId()%>">
                        <input readonly value="<%= order.getFormatedId()%>">
                    </p>
                    <p>
                        <label>轉帳銀行: </label>
                    ...
                    <p>
                        <label>轉帳金額: </label>
                        <input required type="number" min="1" name="amount" value="<%= request.getParameter("amount") == null ? Math.round(order.getTotalAmountWithFee()) : request.getParameter("amount")%>" >
                    </p>                    
                    <p>
                        <label>轉帳日期: </label>
                        <input  type="date" required name="date" min="<%= order.getOrderDate()%>" max="<%= LocalDate.now()%>" value="${param.date}">
