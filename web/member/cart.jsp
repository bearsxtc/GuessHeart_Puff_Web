...
//                                            out.print(ob.format(b));
                                    ...
                                    <!--數量 -->
                                    <td>
                                        <input onchange="checkQuantity(this)"  <%= stock < cart.getQuantity(item) ? "class='stockShortage'" : ""%>  id="quantity"
                                               style="width:3em" type="number" name="quantity_<%= item.hashCode()%>"  
                                               value="<%= cart.getQuantity(item)%>" min="<%= p.getStock() > 0 ? 1 : 0%>" max="<%= p.getStock()%>"> 
                                    </td>
                                    <!--小計 -->
                                    <%
...
//                                            out.print(obj.format(a));
                                    ...
                                    <!--刪除 -->
                                    <td>
                                        <button name="delete_<%= item.hashCode()%>" style="border-style: none; background: white">
                        ...
                        <div class="check_out" style="width: 90%; ">
                            <h4 style="text-align: right" >總金額： <small>NT$</small>
                                <input id="total" value="<%= cart.getTotalAmount()%>"  
                                       style="color: red;font-size: larger; font-weight: 900;border-style: none; font-size: larger;
                                       width:50px; text-align: center;" readonly>
                                元
                            </h4>
                        </div>
                        <div class="btn" style="width: 80%; min-width: 500px; margin: 30px auto;opacity: 0.8">
                            <button value="繼續猜心" name="submit" id="goShoppingButton" style="margin: 5px; padding: 5px; background: white; border: 1px solid black; border-radius: 8px; vertical-align: middle;font-weight: 700;font-family: '微軟正黑體' ">
                                <i class="fas fa-angle-double-left"></i>
                                <span>繼續猜心</span>
                            </button>

                            <button value="確認結帳" name="submit" style="margin: 5px; padding: 5px; background: white; border: 1px solid black; border-radius: 8px; vertical-align: middle; float: right;font-weight: 700;font-family: '微軟正黑體' " >
                                <span>確認結帳</span>
